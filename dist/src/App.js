...
function getRequiredMonthlyExpenseReduction(input) {
  if (!calculateProjection(input).depletionAge) return 0;
  let low = 0;
  let high = Math.max(Number(input.monthlyExpenses) || 0, Number(input.retirementExpenses) || 0, 10000);
  for (let index = 0; index < 24; index += 1) {
    const mid = (low + high) / 2;
    const testInput = {
      ...input,
      monthlyExpenses: Math.max(0, (Number(input.monthlyExpenses) || 0) - mid),
      retirementExpenses: Math.max(0, (Number(input.retirementExpenses) || 0) - mid),
    };
    if (calculateProjection(testInput).depletionAge) {
      low = mid;
    } else {
      high = mid;
    }
...
function getOneManReductionYears(input, projection) {
  ...
  const testProjection = calculateProjection({
    ...input,
    monthlyExpenses: Math.max(0, (Number(input.monthlyExpenses) || 0) - 10000),
...
function getRetirementExpenseIncreaseByReturn(input) {
  if (calculateProjection(input).depletionAge) return 0;
  const improvedInput = withLinkedReturnFields(input, (Number(input.annualReturn) || 0) + 1);
  let low = 0;
  let high = Math.max(Number(input.retirementExpenses) || 0, 10000);
  while (!calculateProjection({ ...improvedInput, retirementExpenses: (Number(input.retirementExpenses) || 0) + high }).depletionAge && high < 3000000) {
    high *= 2;
  }
  for (let index = 0; index < 24; index += 1) {
    const mid = (low + high) / 2;
    const testInput = { ...improvedInput, retirementExpenses: (Number(input.retirementExpenses) || 0) + mid };
    if (calculateProjection(testInput).depletionAge) {
      high = mid;
    } else {
      low = mid;
...
function getSummaryText(projection, deficitStartAge) {
  const finalBalance = projection.rows.at(-1)?.closingAssets ?? 0;
...
function getShareText(projection, deficitStartAge) {
  const url = "https://life-planning-simulator.vercel.app/";
  return [
    "あなたの将来資産の予測結果は以下のとおりです。",
    "",
    getSummaryText(projection, deficitStartAge),
    "",
    "将来資産シミュレーター",
    url,
    "",
    "１分で将来資産を予測。",
    "最大50項目の入力項目でより精緻なシミュレーションも可能",
  ].join("\n");
}
...
function ResultHintCard({ projection, forecastScenario, requiredMonthlyReduction, oneManReductionYears, retirementExpenseIncrease }) {
  ...
  return h(
    ...
    isDepleted
      ...
      : [
          h("strong", { key: "title" }, `資産運用のヒント：資産（金融資産）の運用利回りが1％上がると老後支出は${retirementExpenseIncrease.toLocaleString("ja-JP")}万円上げることができます。`),
          h("span", { key: "body" }, "詳細入力で資産の運用方法等を見直して余裕のある老後をシミュレーションしましょう。"),
        ],
  );
}
...
const infoPages = {
  ...
  commentary: {
    ...
    lead: "本ツールの考え方",
    paragraphs: [
      "本ツールは、ツール開発者自らの資産管理に対する考え方に基づき開発したものです。",
      "実際には、世界情勢や自らの環境変化等さまざまな要因が資産に影響を与えます。",
      "これらすべてを考慮に入れたうえで自身で計算ソフトを用いて資産推移等を計算することは易しくありません。",
      "また、家や車を買えば現金はなくなりますが、その分資産として残ります。",
      "しかしながら、その資産価値は永遠ではなく、基本的に年数に応じて市場価値は下がります。",
      "本ツールは、簡単な入力情報で将来の資産を簡易的にシミュレーションできるものですが、インフレ率や各種資産の減価設定、ローン等の詳細な情報を入力することにより、より実生活に即したものとすることができます。",
      "ただし、表示された結果は、本ツールの開発者が設定した条件等により将来資産を参考として算出したものであり、将来を保証してくれるものではないことに注意が必要です。",
    ],
  },
  disclaimer: {
    title: "免責事項",
...
function InfoPage({ page, onBack }) {
  ...
  return h(
    ...
    content.paragraphs
      ? h("div", { className: "commentaryBody" }, content.paragraphs.map((paragraph) => h("p", { key: paragraph }, paragraph)))
      : null,
...
function AssetChart({ data, showCashflow, cashflowScale, onStepCashflowScale, onOpen }) {
  const width = 760;
  const height = 380;
  const padding = { top: 62, right: showCashflow ? 64 : 18, bottom: 28, left: 62 };
  ...
  return h(
    ...
    h(
      ...
      h(AxisTitle, { x: 10, y: 18, anchor: "start", title: "資産", detail: "（折れ線）", className: "axisTitle" }),
      h("text", { x: padding.left - 18, y: 48, textAnchor: "end", className: "axisUnit" }, "万円"),
      showCashflow
        ...
        : null,
      showCashflow ? h(AxisTitle, { x: width - 6, y: 18, anchor: "end", title: "収支", detail: "（棒グラフ）", className: "cashAxisTitle" }) : null,
      showCashflow ? h("text", { x: width - 6, y: 48, textAnchor: "end", className: "cashAxisUnit" }, "百万円") : null,
...
function App() {
  ...
  const retirementExpenseIncrease = useMemo(
    () => (!projection.depletionAge ? getRetirementExpenseIncreaseByReturn(forecastScenario) : 0),
    [forecastScenario, projection.depletionAge],
  );
  ...
  function shareToX() {
    const text = encodeURIComponent(getShareText(projection, deficitStartAge));
    window.open(`https://twitter.com/intent/tweet?text=${text}`, "_blank", "noopener,noreferrer");
  }

  function shareToFacebook() {
    const quote = encodeURIComponent(getShareText(projection, deficitStartAge));
    const url = encodeURIComponent("https://life-planning-simulator.vercel.app/");
    window.open(`https://www.facebook.com/sharer/sharer.php?u=${url}&quote=${quote}`, "_blank", "noopener,noreferrer");
  }

  function renderShareButtons(className = "") {
    return h(
      "div",
      { className: `shareButtons ${className}`.trim(), "aria-label": "SNSで共有" },
      h("button", { type: "button", className: "shareButton xShareButton", onClick: shareToX, "aria-label": "Xで共有" }, "X"),
      h("button", { type: "button", className: "shareButton facebookShareButton", onClick: shareToFacebook, "aria-label": "Facebookで共有" }, "f"),
    );
  }
    ...
    h(
      ...
      h(
        ...
        h(
          ...
          h("button", { type: "button", className: activePage === "commentary" ? "pageNavButton active" : "pageNavButton", onClick: () => setActivePage("commentary") }, "解説"),
        ...
        h(
          ...
          h("button", { type: "button", onClick: exportScenarioFile, className: "saveButton fileSaveButton", "aria-label": "現在のシナリオを端末に保存" }, "端末に保存"),
        ),
        renderShareButtons("pcShareButtons"),
      ),
    ),
    activePage !== "app"
      ...
      h(
        ...
        h(
          "div",
          { className: "stepGuide" },
          h("strong", null, "かんたん３ステップ（ログイン不要）"),
          h("span", null, "①情報入力"),
          h("span", null, "②「将来予測」押す"),
          h("span", null, "③結果確認"),
        ),
      ...
      renderShareButtons("mobileShareButtons"),
      h(
        "div",
        ...
        showDefaultDetailNotice
          ? h(
              "p",
              { className: "detailNotice bottomDetailNotice" },
              "入力した項目以外は一般的な数値・条件等により予測しています。",
              h("br"),
              "より精緻な予測をするためには詳細な情報を入力してください。",
            )
          : null,
        h("button", { type: "button", className: "mobileReturnButton", onClick: () => document.querySelector(".inputPanel")?.scrollIntoView({ behavior: "smooth", block: "start" }) }, "再度入力する"),
      ),
    ),
    isScenarioNameOpen
      ? h(
